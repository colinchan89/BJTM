...
function dealRound(){
  switch(dealRoundCounter){
    ...
    default:
      ...
      $('#dealerCard2').text(" ");
      // $('#dealerCard2').css("background", "red");
...
function dealToDealer() {
  var z = 2;
  dealer.addCard(getNextCard());
  $('.dcardcontainer').last().append('<div class="hcard" id="dealerHitCard"'+z+'></div>');
...
function endRound() {
  ...
  if(player.blackjack && !dealer.blackjack){
    ...
    // functionality for score goes here!
  }
  else if ((p <= 21 && d > 21) || (p <= 21 && p > d)){
    alert("You Win!");
    // functionality for score goes here!
    break;
  }
